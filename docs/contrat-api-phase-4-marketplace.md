...
| GET | `/etablissements/{id}/marketplace/annonces` | Élève ≥16 ans de cet établissement, **ou** A+ de cet établissement | Catalogue, filtres optionnels `?categorie=&etat=&prix_min=&prix_max=`, pagination standard, statut `disponible` par défaut (`?statut=` pour élargir). **Correction (étape 6, intégration frontend)** : l'A+ doit pouvoir consulter le catalogue de sa propre initiative — UC-20 prévoit qu'il puisse retirer une annonce "sans devoir attendre un signalement", ce qui suppose cet accès en lecture, absent du premier jet du contrat |
| GET | `/marketplace/annonces/{id}` | Élève ≥16 ans du même établissement que l'annonce, **ou** A+ de cet établissement | Lecture, photos résolues en liens signés LuluFiles à la demande (ADR-009), jamais en masse |
