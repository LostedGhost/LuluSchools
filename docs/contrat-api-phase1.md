...
| POST | `/etablissements/{id}/localisation` | A++ (tout établissement) ou A+ (le sien uniquement) | — | Corrige/renseigne `latitude`/`longitude` après coup — typiquement l'A+ physiquement sur place via la géolocalisation du navigateur (saisie manuelle en secours), ou l'A++ à distance |
