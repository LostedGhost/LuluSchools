...
Étapes 1 (cas d'utilisation), 2 (UML) et 3 (choix technique et contrat d'API) validées. Étape 4 (backend) : tous les modules de la Phase 1 (identité, établissements, inscriptions, recrutement/contrats, pédagogie, évaluations, actes académiques) faits et testés — 67 tests passants, 10 migrations appliquées. Décisions récentes : signature du contrat par tracé manuel sur canvas (ADR-004), moyennes pondérées par les coefficients, quiz généré par IA, devoirs sous forme de formulaires corrigés par IA, écrans de révision manuelle, webhook de paiement Kkiapay (URL unique par compte). Détail dans [backend/PROJECT_MAP.md](backend/PROJECT_MAP.md) et [docs/contrat-api-phase1.md](docs/contrat-api-phase1.md). Étape 5 (validation de bout en bout) pas encore commencée.
