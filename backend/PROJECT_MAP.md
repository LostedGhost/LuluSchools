...
- `GET /etablissements/vitrine-publique` — **seul endpoint métier sans authentification de tout le backend** (le seul autre précédent est `GET /health` dans `app/system/router.py`), déclaré lui aussi avant `GET /{etablissement_id}` pour la même raison d'ordre de routes. Alimente la section vitrine de la landing page (établissements partenaires + postes enseignants ouverts) : n'expose que des champs non sensibles (`EtablissementVitrineOut`/`PosteVitrineOut`/`VitrinePubliqueOut` dans `schemas.py`, jamais `code_etablissement` ni d'email d'admin). Importe `Poste`/`StatutPoste` depuis `modules/recrutement/models.py` (couplage en lecture seule, cohérent avec l'import déjà existant de `modules/messagerie/models`).
