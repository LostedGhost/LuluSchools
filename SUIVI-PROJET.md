...
- [~] **6. Frontend** — parcours Tuteur et Élève construits et **testés en navigateur réel contre le backend réel** (pas de mocks, voir `frontend/PROJECT_MAP.md`) — a révélé et corrigé un second vrai bug (`DocumentCandidatureOut` sans `id`). Restent : Enseignant, A+, A++, écrans de révision manuelle.
- [~] **7. Intégration** — déjà de facto pour le parcours Tuteur/Élève (le frontend n'a jamais utilisé de mocks, connecté au backend réel dès le départ) ; à compléter au fur et à mesure des rôles restants
